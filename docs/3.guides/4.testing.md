...
For `Database[T]` stores, you often want to confirm the SQL that grub/soy builders
generate — "the query is built as we expect" — without standing up a live database.
The `grub/mockdb` package provides a mock-backed `*sqlx.DB` that captures every
query and its args.
...
func TestUserQueries(t *testing.T) {
    ...
    query, ok := capture.Last() // most recent CapturedQuery: SQL + args
    if !ok {
        t.Fatal("no query captured")
    }
    if !strings.Contains(query.Query, `SELECT`) {
        t.Errorf("expected SELECT, got: %s", query.Query)
    }
}
```

Use `mockdb.NewWithConfig()` to also drive return rows and error paths:
...
cfg.SetExecErr(errors.New("boom")) // force the exec path to fail
```

### Multi-Query Methods

`SetRowData`/`SetExecErr` set a single response returned for every query in a
run. A store method that issues **several queries in one call** — a locking read,
then a count, then an insert — needs a different response per step. Use the
`Push*` methods to enqueue responses consumed in order, one per query/exec:

```go
mockDB, _, cfg := mockdb.NewWithConfig()

cfg.PushRowData(&mockdb.RowData{ // query 1: the FOR UPDATE read
    Columns: []string{"id", "status"},
    Rows:    [][]any{{int64(1), "draft"}},
})
cfg.PushRowData(&mockdb.RowData{ // query 2: the COUNT
    Columns: []string{"count"},
    Rows:    [][]any{{int64(3)}},
})
cfg.PushExecErr(errors.New("dup key")) // the INSERT then fails
```

Each `Push*` call appends to a queue (`PushRowData`, `PushQueryErr` for reads;
`PushExecErr`, `PushRowsAffected` for writes). When a queue is exhausted,
behavior falls back to the single `Set*` configuration. This lets a whole
multi-query method's generated SQL be asserted via `capture` without a live DB.

Import `grub/mockdb` only from test code: importing it registers the mock SQL
driver, and production binaries that never import it pull in none of it.
